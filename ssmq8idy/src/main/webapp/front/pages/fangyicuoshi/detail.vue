...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','fangyicuoshi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','fangyicuoshi');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('fangyicuoshi', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussfangyicuoshi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('fangyicuoshi', this.detail);
				this.$utils.msg('审核成功');
