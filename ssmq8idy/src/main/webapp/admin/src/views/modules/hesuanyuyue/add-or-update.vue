<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="检测余量" prop="jianceyuliang">
          <el-select :disabled="ro.jianceyuliang" v-model="ruleForm.jianceyuliang" placeholder="请选择检测余量">
            <el-option
                v-for="(item,index) in jianceyuliangOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="检测余量" prop="jianceyuliang">
	      <el-input v-model="ruleForm.jianceyuliang"
                placeholder="检测余量" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        this.ruleForm.jianceyuliang = 0
	    this.ro.jianceyuliang = false;
      }
      // 获取用户信息
      this.$http({
		...
		if(json.xingming!=''&&json.xingming){
          ...
          this.$message.error(data.msg);
            ...
            this.jianceyuliangOptions = "1".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
...
var objcross = this.$storage.getObj('crossObj');
      ...
      if(objcross!=null) {
	      objcross.jianceyuliang = objcross.jianceyuliang - this.ruleForm.jianceyuliang
	      if(objcross.jianceyuliang<0){
		this.$message.error("检测余量不足");
		return
	      }
                }
	      this.$http({
